...
public class OrderDAO {
	...
	public List getOrderList(int page,int limit,String id) throws SQLException {
		String sql="select * from (select rownum rnum,ORDER_GOODS_NUM,"+
				"ORDER_GOODS_NAME,ORDER_GOODS_AMOUNT,ORDER_SUM_MONEY,ORDER_DATE,"+
	...
	public int addOrder(OrderBean order, Vector goodsvector){
		...
		for (int i = 0; i < basketlist.size(); i++) {
			...
			try {
				...
				pstmt.setString(7, order.getORDER_MEMBER_ID());
				pstmt.setString(8, order.getORDER_RECEIVE_NAME());
				pstmt.setString(9, order.getORDER_RECEIVE_ADDR1());
				pstmt.setString(10, order.getORDER_RECEIVE_ADDR2());
				pstmt.setString(11, order.getORDER_RECEIVE_PHONE());
				pstmt.setString(12, order.getORDER_RECEIVE_MOBILE());
				pstmt.setString(13, order.getORDER_MEMO());
				pstmt.setInt(14, (goods.getGOODS_PRICE()*basket.getBASKET_GOODS_AMOUNT()));
				pstmt.setString(15, order.getORDER_TRADE_TYPE());
				pstmt.setString(16, order.getORDER_TRADE_PAYER());
